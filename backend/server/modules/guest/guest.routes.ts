 ...
 */
function handleRangeRequest(req: any, res: any, filePath: string, contentType: string): { start: number; end: number; fileSize: number } | null {
...
router.get('/by-title/stream', asyncHandler(async (req, res) => {
  ...
  const stream = fs.createReadStream(absPath, streamOptions);
  stream.on('error', (err) => {
 ...
 *         description: Трек не найден
 */
router.get('/nfc/stream', asyncHandler(async (req, res) => {
  const title = req.query.title as string;
  ...
  // Путь к папке NFC
  const nfcDir = path.resolve(appConfig.storageRoot, 'nfc');
  ...
  const files = fs.readdirSync(nfcDir);
  ...
  const foundFile = files.find(file => {
    ...
    if (fileNameWithoutExt === titleLower) {
      console.log(`[NFC-STREAM] Match: exact name match "${fileNameWithoutExt}" === "${titleLower}"`);
      return true;
    }
    
    // 2. Точное совпадение полного имени файла
    if (fileLower === titleLower) {
      console.log(`[NFC-STREAM] Match: exact file match "${fileLower}" === "${titleLower}"`);
      return true;
    }
    
    // 3. Проверка с расширениями
    for (const ext of audioExtensions) {
      if (fileLower === `${titleLower}${ext}`) {
        console.log(`[NFC-STREAM] Match: with extension "${fileLower}" === "${titleLower}${ext}"`);
        return true;
      }
    }
    
    // 4. Файл начинается с title (например, "dream" найдет "dreams")
    if (fileNameWithoutExt.startsWith(titleLower)) {
      console.log(`[NFC-STREAM] Match: file starts with title "${fileNameWithoutExt}".startsWith("${titleLower}")`);
      return true;
    }
    
    // 5. Title начинается с имени файла (например, "dreams" найдет "dream")
    if (titleLower.startsWith(fileNameWithoutExt)) {
      console.log(`[NFC-STREAM] Match: title starts with file "${titleLower}".startsWith("${fileNameWithoutExt}")`);
      return true;
    }
    
    // 6. Если файл начинается с "en", проверяем без этого префикса
    if (fileNameWithoutExt.startsWith('en')) {
      const fileWithoutEn = fileNameWithoutExt.substring(2);
      if (fileWithoutEn === titleLower || fileWithoutEn.startsWith(titleLower) || titleLower.startsWith(fileWithoutEn)) {
        console.log(`[NFC-STREAM] Match: file with "en" prefix "${fileWithoutEn}" matches "${titleLower}"`);
        return true;
      }
    }
    
    // 7. Если title начинается с "en", проверяем файл без этого префикса
    if (titleLower.startsWith('en')) {
      const titleWithoutEn = titleLower.substring(2);
      if (fileNameWithoutExt === titleWithoutEn || fileNameWithoutExt.startsWith(titleWithoutEn) || titleWithoutEn.startsWith(fileNameWithoutExt)) {
        console.log(`[NFC-STREAM] Match: title with "en" prefix "${titleWithoutEn}" matches "${fileNameWithoutExt}"`);
        return true;
      }
    }
  ...
  const absPath = path.resolve(nfcDir, foundFile);
  ...
  const contentType = mime.lookup(ext) || 'audio/mpeg';
  const stat = fs.statSync(absPath);
